# ============================================================
# ecNodales_Frio_Heaters.py — Fase 3
...
get_potencia = props['get_potencia']

# ==========================================================
# Estados del heater (persistentes en el módulo)
# ==========================================================
heater12_on = False
heater13_on = False
...
# ==========================================================
# Helpers de ventanas angulares (idéntico a tu versión actual)
# ==========================================================
def _theta_mask_sol(theta_deg):
    return ((THETA_C1 < theta_deg < THETA_C2) or (THETA_C3 < theta_deg < THETA_C4))

def _theta_mask_alb(theta_deg):
    return ((0 < theta_deg < THETA_C1) or (THETA_C4 < theta_deg < 360))

def _cos_phi(theta_deg):
    return -np.cos(np.radians(theta_deg))


# ==========================================================
# Núcleos de cálculo (idéntico a tu modelo actual)
# ==========================================================
...
# ==========================================================
# NODOS 12 Y 13 — CON HEATERS ACTUANDO
# ==========================================================
def _box_common_heaters(i, T, dt, cond_row, fv_row, area, masa, cp, nodo_id, theta_deg):
    global heater12_on, heater13_on

    Ti = T[i]

    # --- Conducción + radiación interna ---
    q_cond = sum(c * (T[n] - Ti) for n, c in enumerate(cond_row))
    q_rad  = sum(EPS_AL * SIGMA * fv * area * (T[n]**4 - Ti**4) for n, fv in enumerate(fv_row))

    # --- Potencia interna (0 en frío) ---
    pot_dis = get_potencia(theta_deg, nodo_id)

    # --- HEATERS (histeresis ON/OFF real) ---
    if nodo_id == 12:    # nodo 12
        if not heater12_on and Ti < T12_ON:
            heater12_on = True
        elif heater12_on and Ti > T12_OFF:
            heater12_on = False
        if heater12_on:
            pot_dis += P_HEATER_12

    if nodo_id == 13:    # nodo 13
        if not heater13_on and Ti < T13_ON:
            heater13_on = True
        elif heater13_on and Ti > T13_OFF:
            heater13_on = False
        if heater13_on:
            pot_dis += P_HEATER_13
...
def ecNodo12(T, dt, cond_row, fv_row, theta):
    return _box_common_heaters(11, T, dt, cond_row, fv_row, AREA_OBC, MASA_OBC, CP_OBC, 12, theta)

def ecNodo13(T, dt, cond_row, fv_row, theta):
    return _box_common_heaters(12, T, dt, cond_row, fv_row, AREA_BAT, MASA_BAT, CP_BAT, 13, theta)